(function() {
    ...
    function updateUI(ae, bl) {
        var r = document.getElementById('ae-result');
        var c = document.getElementById('calendly-container');
        if (!r || !c) return;
        if (ae) {
            r.innerHTML = '<div class="ae-info"><p>Zuständiger Account Executive für ' + bl + '</p><p><strong>Name:</strong> ' + ae.name + '</p></div>';
            if (ae.calendlyLink) {
                c.innerHTML = '<div class="calendly-inline-widget" data-url="' + ae.calendlyLink + '?hide_gdpr_banner=1&hide_event_type_details=1&hide_landing_page_details=1&background_color=ffffff&hide_title=1" style="min-width:320px;height:700px;"></div>';
                if (window.Calendly) {
                    window.Calendly.initInlineWidget({
                        ...
                        parentElement: c.querySelector('.calendly-inline-widget')
                    });
                }
            } else {
                c.innerHTML = '<div class="calendly-placeholder">Kein Kalenderlink verfügbar.</div>';
            }
        } else {
            c.innerHTML = '<div class="calendly-placeholder">Bitte wählen Sie zuerst ein Bundesland aus, um den Kalender zu laden.</div>';
        }
    }
    ...
    function init() {
        ...
        var bs = document.getElementById('bundesland-select');
        ...
        if (f) {
            f.addEventListener('submit', async function(e) {
                e.preventDefault();
                var sb = f.querySelector('.ios-submit');
    ...
    function loadDependencies() {
        ...
        ps.src = 'https://cdnjs.cloudflare.com/ajax/libs/PapaParse/5.3.0/papaparse.min.js';
        ps.onload = function() {
    ...
    window.addEventListener('message', function(e) {
        if (e.data.event === 'calendly.event_scheduled') {
            var em = e.data.payload && e.data.payload.invitee && e.data.payload.invitee.email;
            var ei = document.getElementById('email-field');
            if (ei && em) ei.value = em;
            var frm = document.getElementById('contact-form');
            var hn = document.getElementById('form-hint');
            if (frm) {
                frm.style.display = 'block';
                setTimeout(function(){frm.style.opacity='1'},10);
            }
