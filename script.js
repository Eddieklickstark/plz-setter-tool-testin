(function() {
    ...
    function updateUI(ae, bundesland) {
        ...
        resultDiv.innerHTML = `
          <div class="ae-info">
            <p>Zuständiger Account Executive für ${bundesland}</p>
            <p><strong>Name:</strong> ${ae.name}</p>
          </div>`;
        calendlyDiv.innerHTML = `<div class="calendly-inline-widget" style="min-width:320px;height:700px;"></div>`;
        window.Calendly.initInlineWidget({
            ...
            onEventScheduled: function(event) {
                var email = event.data.payload.invitee.email;
    ...
    function init() {
        ...
        document.getElementById('bundesland-select').addEventListener('change', function() {
            ...
            if (bl) updateUI(aeMapping[bl], bl);
        });
