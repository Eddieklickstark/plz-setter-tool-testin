...
.bundesland-input-container { margin-bottom: 20px; }
...
<form id="contact-form" class="form-section">
  ...
  <div class="form-group">
    ...
    <div class="form-grid">
      ...
      <input type="text" class="ios-input required" name="hausnummer" placeholder="Hausnummer*" required>
      <input type="text" class="ios-input required" name="plz" placeholder="Postleitzahl*" required>
  ...
  <div class="form-group">
    ...
    <div class="form-grid">
      ...
      <input type="url" class="ios-input" name="linkedin" placeholder="LinkedIn Profil (https://…)" style="grid-column: span 2;">
    </div>
  </div>
  <div class="form-group">
    <h3 class="subsection-header">Gesprächsnotiz*</h3>
    <textarea class="ios-input ios-textarea required" name="gespraechsnotiz" placeholder="Gesprächsnotiz – mind. 3 Sätze" required></textarea>
  </div>
  <button type="submit" class="ios-submit">Informationen senden</button>
  <div class="success-message" id="success-message"><p>Daten wurden erfolgreich gespeichert!</p><p>Seite wird neu geladen…</p></div>
</form>
<div class="overlay" id="loading-overlay"><div class="spinner"></div></div>
  ...
  function updateBundeslandSelect() {
    const sel = document.getElementById('bundesland-select');
    if (!sel) return;
      ...
      if (xhr.readyState === 4 && xhr.status === 200) {
        Papa.parse(xhr.responseText, {
          ...
          complete: res => {
            ...
            res.data.forEach(r => {
              if (r.Bundesland && r.name) {
                const bl = r.Bundesland.trim();
                aeMapping[bl] = {
                  name: r.name.trim(),
                  calendlyLink: (r.calendly_link||'').trim()
                };
